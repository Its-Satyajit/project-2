...
export function Navigation() {
  ...
  const callDebugAction = async (action: string) => {
    ...
    if (res.error) {
      const errorMsg =
        typeof res.error === "object" && res.error !== null && "value" in res.error
  ...
  const triggerAnalysis = async () => {
    ...
    await handleAction("Trigger analysis", async () => {
      ...
      if (res.data && typeof res.data === "object" && "owner" in res.data && "name" in res.data) {
            ...
            <>
              ...
              <Link
                className={`group flex items-center gap-2 rounded-md px-3 py-1.5 text-sm transition-all ${
                  pathname === `/dashboard/${repoId}` ? "bg-accent" : "hover:bg-accent"
                }`}
              ...
              <Link
                className={`group flex items-center gap-2 rounded-md px-3 py-1.5 text-sm transition-all ${
                  pathname === `/dashboard/${repoId}/analysis` ? "bg-accent" : "hover:bg-accent"
                }`}
