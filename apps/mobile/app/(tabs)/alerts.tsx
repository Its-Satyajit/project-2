...
export default function AlertsScreen() {
	...
	const filtered = ALERTS.filter((a) => {
		...
		if (filter === 2)
			return a.type === "hotspot_detected" || a.type === "ci_failure";
	...
	const getIcon = (type: string) => {
		switch (type) {
			case "ci_failure":
				return <AlertTriangle color={colors.status.error} size={20} />;
			case "hotspot_detected":
				return <Flame color={colors.status.warning} size={20} />;
			case "analysis_complete":
				return <CheckCircle color={colors.status.success} size={20} />;
			default:
				return <Bell color={colors.accent.primary} size={20} />;
		}
	};
	...
	return (
		...
		>
			...
			<TouchableOpacity
				style={[styles.pushCard, { backgroundColor: colors.accent.muted }]}
			>
				<BellRing color={colors.accent.primary} size={20} />
				<View style={styles.pushContent}>
					<Text style={[styles.pushTitle, { color: colors.text.primary }]}>
						Push Notifications
					</Text>
					<Text style={[styles.pushDesc, { color: colors.text.muted }]}>
						Tap to enable
					</Text>
				</View>
			</TouchableOpacity>
			...
			<View style={styles.filters}>
				{FILTERS.map((f, i) => (
					<TouchableOpacity
						...
						style={[
							styles.filter,
							filter === i && { backgroundColor: colors.accent.primary },
						]}
					>
						<Text
							style={[
								...
								filter === i && { color: colors.background, fontWeight: "700" },
							]}
						>
							{f}
						</Text>
			...
			{filtered.length === 0 ? (
				<EmptyState description="No alerts" icon={Bell} title="All caught up" />
			) : (
					...
					{filtered.map((alert) => (
						...
						>
							...
							<View style={styles.alertContent}>
								<Text
									style={[styles.alertTitle, { color: colors.text.primary }]}
								>
									{alert.title}
								</Text>
								<Text style={[styles.alertRepo, { color: colors.text.muted }]}>
									{alert.repo}
								</Text>
